			...
			$("#btnSubmit").click(function(){
					if($("#inputForm").valid()){
						...
						$("input[type='checkbox'][name='delId']:checked").each(function(){
							...
							*/
 							if(""==$("#dtQuitInsurance"+this.value).val()){
								isNullNum ++ ;
								return ;
							} 
							 ...
							 var s = new Date($("#dtAudit"+this.value).val()).getTime(); 							 
							 var e = new Date($("#dtQuitInsurance"+this.value).val()).getTime(); 
...
/* 							  if(now>s){
							  ...
							  if(now>=e){
								  isNullNum ++ ;
								  msg= $("#workerName"+this.value).val()+" ，的退保日期必须在今天之后（包含今天）！" ;
								  $("#dtQuitInsurance"+this.value).val("");
								  return ;
							  }
 							 if(s>e){
								 isNullNum ++ ;
						...
						/* 	if(""==insuranceTps){
							...
							if(""==sDates){
								sDates = this.value+"|"+ $("#dtAudit"+this.value).val() ;
							}else{
								sDates = this.value+"|"+ $("#dtAudit"+this.value).val()+","+sDates;
							} 
						
							if(""==eDates){
								eDates = this.value+"|"+ $("#dtQuitInsurance"+this.value).val() ;
							}else{
								eDates = this.value+"|"+ $("#dtQuitInsurance"+this.value).val()+","+eDates;
							}
						...
						if(workerIds.length == 0){
							...
							$("#inputForm").submit(
									 $.ajax({
									 	type : 'post',
										url :  '${ctx}/taogr/worker/tbInsuranceList/saveQuit',
										data : {
											workerIds : workerIds,
...
<!-- 				<th>险种</th>
		...
		<c:forEach items="${tbEntWorkerRelList}" var="tbEntWorkerRel" varStatus="statuss">
			<input type="hidden" name="worker.id" id="workerId"
						 value="${tbEntWorkerRel.worker.id}"/>
			<tr>
...
<%-- 				<td>
				...
				<td>
				<input id="dtAudit${tbEntWorkerRel.worker.id}" name="dtAudit" type="text" style="width:120px;"  maxlength="20" class="input-medium"
					value="${tbEntWorkerRel.tbInsuranceList.dtAudit}"
					onclick=""/>
				</td> 
				<td>
				<input id="dtQuitInsurance${tbEntWorkerRel.worker.id}" name="dtQuitInsurance" type="text"  style="width:120px;" maxlength="20" class="input-medium Wdate"
					value=""
	...
	<ul class="ul-form">
	<li class="btns">
	 <a class="btn btn-success" id="btnSubmit">退保申请</a> 
