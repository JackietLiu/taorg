			...
			$("#confirmInsBatch").click(function() {
				...
				if(data.length == 0) {
					...
					var submit = function (v, h, f) {
		                if (v == true){
		                	window.location.href="${ctx}/taogr/worker/tbInsuranceList/updateStatusAuditBatch?ids="+data;
		                }else{
					...
					jBox.confirm("执行确认保单吗？", "提示", submit, { id:'hahaha', showScrolling: false, buttons: { '执行': true, '取消': false } });
				}
			});
...
<body>
	...
	<form:form id="searchForm" modelAttribute="tbInsuranceList" action="${ctx}/taogr/worker/tbInsuranceList/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
				...
				<a class="btn btn-success" id="confirmInsBatch">确认保单</a>
				</shiro:hasPermission>
				</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			...
			<th class="span1" style="text-align: center; "><input type="checkbox" name="allcheck" id="allcheck" /></th>
				<th>企业</th>
				<th>工人姓名</th>
				<th>身份证</th>
				<th>保单类型</th>
		...
		<c:forEach items="${page.list}" var="tbInsuranceList"  varStatus="statuss">
			...
			<td style="text-align: center; "> <input type="checkbox" name="delId" id="score${statuss.count}" value="${tbInsuranceList.id}"/></td>
				...
				<td>
					<%-- <a href="${ctx}/taogr/worker/tbInsuranceList/updateStatusAudit?id=${tbInsuranceList.id}&statusAudit=3" onclick="return confirmx('确认要取消保单信息吗？', this.href)">取消保单</a> --%>
					
					<c:if test="${tbInsuranceList.statusAudit==1 }"><!-- 未确认保单并且没有退保的，才能操作确认保单 ，即使申请了退保，如果没有退保确认，也能继续确认保单-->
