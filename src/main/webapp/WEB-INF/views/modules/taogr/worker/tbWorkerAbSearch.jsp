						...
						/*  $("#quit").click(
								...
								$("#search_type").click(function(){
									...
									if(this.value==1){
										...
										window.location.href="${ctx}/taogr/worker/tbEntWorkerRel/ability";
									}else{
										window.location.href="${ctx}/taogr/worker/tbEntWorkerRel/list";
									}
		...
		<tbody>
			<c:forEach items="${page.list}" var="tbEntWorkerRel"
				...
				<tr>
					...
					<td><%-- <c:if test="${tbEntWorkerRel.tbInsuranceList.inQuit == 1}">
					
				    </c:if>  --%> ${tbEntWorkerRel.insuranceTpCd.insName}
					</td>
			...
			<%--    <c:if test="${tbEntWorkerRel.tbInsuranceList.inQuit.indexOf('1') > 0 || tbEntWorkerRel.tbInsuranceList.inQuit == 1}"><!-- 未退保，才可以申请退保 -->
					...
					<a href="${ctx}/taogr/worker/tbInsuranceList/formExit?id=${tbEntWorkerRel.tbInsuranceList.id}"><font color="orange">退保申请</font></a>
						 ...
						 <%-- <a href="${ctx}/taogr/worker/tbEntWorkerRel/form?id=${tbEntWorkerRel.id}">修改</a> --%>
